...
if selectbox == LIST[0]:
    ...
    **Pick first 2 components and transform data:**
    ...
    We assume that there're a $$b \in R^D$$ and a set of datapoint  $$A\in R^{D \times N}$$.
...
elif selectbox == LIST[1]:
    st.title('PCA Tools')
    from PCA import PCA
    row_number = int(st.number_input("Input number of rows:", min_value=2, max_value=1000))
    col_number = int(st.number_input("Input number of columns:", min_value=2, max_value=1000))
    
    A = np.random.randint(-100, 100, (row_number, col_number))

    st.dataframe(A)

    component_number = st.number_input("Input components:", min_value=1, max_value=min(A.shape))

    try:
        pca = PCA(n_components=component_number)
        
        st.dataframe(pca.fit_transform(A))

        st.markdown(f"**Explained Variance:** " + str(pca.explained_variance))
        st.markdown(f"**Explained Variance Ratio:** " + str(pca.explained_variance_ratio))
        st.markdown('**Components:** ')

        i = 1
        for v in pca.components.T:
            vector_latex = f'PC_{i}=' + '\\begin{bmatrix}' + '\\\\'.join(map(str,v)) + '\\end{bmatrix}'
            st.latex(vector_latex)
            i += 1
    except Exception:
        pass

    st.markdown("## Code from scratch (without library)")
    st.code('''
    import numpy as np
    ...
    class PCA:
        def __init__(self, n_components: int):
            assert n_components > 0, "n_components must be > 0"
